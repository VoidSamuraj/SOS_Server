        ...
        return transaction {
            ...
            if (insertStatement.resultedValues?.singleOrNull()?.let(::resultRowToClient) != null)
                return@transaction Triple(
                    ...
                    insertStatement.resultedValues?.firstOrNull()?.let { resultRow -> resultRowToClient(resultRow) })
    ...
    override suspend fun editCustomer(
        ...
        return transaction {
            ...
            if (pesel != null) {
                ...
                if (list.count() > 0 && list.any { customer -> customer.id != id }) {
                    return@transaction "Pesel is already taken." to null
                }
            }
            ...
            if (email != null) {
                ...
                if (list.count() > 0 && list.any { customer -> customer.id != id }) {
                    return@transaction "Email is already taken." to null
                }
            }
